...
if [ "${MODE}" = PKG ]; then
  ...
  curl -sfL $DOCKER_INSTALL_SH | DOCKER_REGISTRY_INNER_NO=false MODE=$MODE ARCH=$ARCH sh
...
# 安装服务
  ...
  if which docker > /dev/null 2>&1; then
    ...
    if $DOWNLOAD_SKIP; then
      cat docker-install/install.sh | DOCKER_REGISTRY_INNER_NO=false DOWNLOAD_SKIP=true DOCKER_ROOT=$DOCKER_ROOT DOCKER_IPTABLES=$DOCKER_IPTABLES sh
    # 在线模式
    else
      curl -sfL $DOCKER_INSTALL_SH | DOCKER_REGISTRY_INNER_NO=false DOCKER_ROOT=$DOCKER_ROOT DOCKER_IPTABLES=$DOCKER_IPTABLES sh
    fi
  fi
